...
abstract class Controller extends BaseController {
    ...
    function postMultisearch( Request $request)
    {
        ...
        return Redirect::to($this->modulo.'?search='.substr($items,0,strlen($items)-1).'&md='.$request->inpuy('md'));
    }
    ...
    function validatePost(  $request )
    {
        ...
        foreach($str as $f){
            ...
            if($field == 'f_creacion') $data['f_creacion'] = date('Y-d-m H:i:s');
            if($field == 'f_modificacion') $data['f_modificacion'] = date('Y-d-m H:i:s');
            if($f['view'] ==1)
            {
    ...
    public function getSearch( $mode = 'native' )
    {
        ...
        $this->data['pageUrl']		= url($this->modulo);
        return view('katana.modulo.utility.search',$this->data);
    ...
    function getImport( Request $request)
    {
        ...
        if($task !='')
        {
            ...
            $this->data['url'] = url($this->modulo) ;
            $this->data['module'] = $this->modulo ;
            return view('katana.modulo.utility.import', $this->data);
        }
...
//                $csv = array_map('str_getcsv', file('./uploads/'.$filename));
                ...
                $csv = array_map(function ($v){return str_getcsv($v, ";");}, file('./uploads/'.$filename));
                ...
                foreach($csv as $index=>$row) {
                        ...
                        {
                            if($key != 0 )
                                $data[$val] = (isset($row[$key]) ? utf8_encode($row[$key]): '' ) ;
                        }
                        //print_r($data);
                        //echo $row[0];
                ...
                return response()->json(array('status'	=> 'success','message'=>'Csv Importado Correctamente !'));
            } else {
                return response()->json(array('status'	=> 'error','message'=>'Un error ha ocurrido al subir el archivo!'));
            }
        } else {
            return response()->json(array('status'	=> 'error','message'=>'Seleccione un archivo para subir!'));
        }
    ...
    public function getExpotion()
    {
        ...
        return view('katana.modulo.utility.export', $this->data);
    }
    public function getExport( Request $request, $t = 'excel')
    {
        ...
        $info 		= $this->model->makeInfo( $this->modulo);
        $filter = '';
        ...
        if($t == 'word')
        {
            return view('katana.modulo.utility.word',$content);

        } else if($t == 'pdf') {

            $html = view('katana.modulo.utility.pdf', $content)->render();
            return \PDF::load($html)->show();

        } else if($t == 'csv') {

            return view('katana.modulo.utility.csv',$content);

        } else if ($t == 'print') {

            //return view('katana.modulo.utility.print',$content);
            $data['html'] = view('katana.modulo.utility.print', $content)->render();
            return view('layouts.blank',$data);

        } else  {

            return view('katana.modulo.utility.excel',$content);
        }
    }
    ...
    function getLookup( $request)
    {
        ...
        if(count($args)>=2)
        {
            ...
            $data['key']		= $info['key'];
            //$data['ids']		= 'md'-$info['id'];
            return view('katana.modulo.utility.masterdetail',$data);
    ...
    public function grab( $request , $args = array())
    {
        ...
        if($this->info['type'] =='ajax'){
            $pagination->setPath( $this->modulo.'/data' );
        } else {
            $pagination->setPath( $this->modulo );
        }
    ...
    {
        $columns_from_db = \DB::select("SP_COLUMNS ".$this->info['table'] );
//        \DB::select("SHOW COLUMNS FROM ".  $this->info['table'] )
        foreach($columns_from_db as $column)
        {
            if( $column->COLUMN_NAME != $this->info['key'])
...
//            \DB::select($sql);
            ...
            return ['message'=>__('Por favor seleccione alguna fila para copiar'),'status'=>'error'];
        }
    ...
    function access( $id )
    {
        $row = \DB::table('tb_grupos_acceso')->where('modulo_id', $id)->where('grupo_id', session('gid') )->get();
