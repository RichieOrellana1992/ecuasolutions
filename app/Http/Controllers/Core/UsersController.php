...
class UsersController extends Controller {
	...
	public function index( Request $request )
	{
		...
		$filter = [
			'params' => " AND tb_grupos.nivel> '".Users::level(session('gid'))."'"
		];
	...
	function create( Request $request ) 
	{
		...
		$this->data['id'] = '';
		return view('core.usuarios.form',$this->data);
	...
	function edit( Request $request , $id ) 
	{
		...
		if(!isset($this->data['row']))
			return redirect($this->modulo)->with('message','Record Not Found !')->with('status','error');
	...
	function show( Request $request , $id ) 
	{
		...
		switch( $task)
		{
			...
			default:
				...
				return view('core.usuarios.view',$this->data);
				break;		
		}
	}	
	...
	function postSave( $request, $id =0)
	{
		...
		if($request->input('id') =='')
		{
			...
			$rules['email'] 				= 'required|email|unique:tb_usuarios';
			$rules['username'] 				= 'required|alpha_num|min:2|unique:tb_usuarios';
		...
		if ($validator->passes()) {
			...
			{
				$return = 'core/usuarios/update/'.$id.'?return='.self::returnUrl();
			} else {
				$return = 'core/usuarios?return='.self::returnUrl();
			}
	...
	function postDoblast( Request $request)
	{
		...
		{	
			...
			if(!is_null($request->input('groups')))
			{
				...
				return redirect('core/usuarios/blast')->with('message','Total '.$count.' Message has been sent')->with('status','success');

			}
			return redirect('core/usuarios/blast')->with('message','No Message has been sent')->with('status','info');
			

		} else {

			return redirect('core/usuarios/blast')->with('message', 'The following errors occurred')->with('status','error')
			->withErrors($validator)->withInput();
	...
	public function getSearch( $mode = 'native')
	{
		...
		$this->data['pageUrl']		= url('core/usuarios');
