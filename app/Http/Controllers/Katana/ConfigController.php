...
class ConfigController extends Controller {
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            if(session('gid') !='1')
                ...
                ->with('message','No tienes acceso a esta pagina !')->with('status','error');
            return $next($request);
        });
	...
	{	
        ...
        $this->data['pageNote']  =   __('core.t_generalsettingsmall');	
		$this->data['active'] = '';
		return view('katana.config.index',$this->data);
	}
...
$val  .= 	"];\n";
			...
			$filename = base_path().'/config/ktn.php';
			$fp=fopen($filename,"w+"); 
			fwrite($fp,$val); 
			fclose($fp); 
			return redirect('katana/config')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('katana/config')->with('message', 'The following errors occurred')->with('status','success')
	...
	public function getEmail()
	{
		...
		$regEmail = base_path()."/resources/views/usuarios/emails/registration.blade.php";
		$resetEmail = base_path()."/resources/views/usuarios/emails/auth/reminder.blade.php";
		$this->data = array(
			'groups'	=> Grupos::all(),
		...
		return view('katana.config.email',$this->data);
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return redirect('katana/config/email')->with('message', 'Email Has Been Updated')->with('status','success');
			
		}	else {

			return redirect('katana/config/email')->with('message', 'The following errors occurred')->with('status','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('katana.config.security',$this->data);
...
$val  .= 	"];\n";
			...
			$filename = base_path().'/config/six.php';
			$fp=fopen($filename,"w+"); 
			fwrite($fp,$val); 
			fclose($fp); 
			return redirect('katana/config/security')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('katana/config/security')->with('message', 'The following errors occurred')->with('status','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('katana.config.log',$this->data);	
	}
	...
	public function getClearlog()
	{
		...
		//return redirect('katana/config/log')->with('message','Cache has been cleared !')->with('status','success');
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('katana.config.translation.'.$template,$this->data);
	}
	
	public function getAddtranslation()
	{
		return view("six.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return redirect('katana/config/translation')->with('messagetect','New Translation has been added !')->with('status','success');	;
			
		} else {
			return redirect('katana/config/translation')->with('message','Failed to add translation !' )->with('status','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return redirect('katana/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return redirect('katana/config/translation')->with('message','Translation has been removed !')->with('status','success');
