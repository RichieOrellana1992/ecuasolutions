...
class SiteHelpers
{
	public static function menus( $position ='top',$active = '1')
	{
		...
		{
			$child_level = array();
			$p = json_decode($row->datos_acceso,true);
...
//			}
			if($is_allow ==1) 
			{
				...
				{	 
					...
					{
						$p = json_decode($row2->datos_acceso,true);
...
//						}
						...
						{						
							...
							$menu2 = array(
									...
									'modulo'		=> $row2->modulo,
									'menu_tipo'		=> $row2->menu_tipo,
									'url'			=> $row2->url,
									'menu_nombre'		=> $row2->menu_nombre,
									'menu_lang'		=> json_decode($row2->menu_lang,true),
									'menu_iconos'	=> $row2->menu_iconos,
									'childs'		=> array()
								);	
								...
								foreach ($menus3 as $row3) 
								{
									...
									{								
										$menu3 = array(
												...
												'modulo'		=> $row3->modulo,
												'menu_tipo'		=> $row3->menu_tipo,
												'url'			=> $row3->url,												
												'menu_nombre'		=> $row3->menu_nombre,
												'menu_lang'		=> json_decode($row3->menu_lang,true),
												'menu_iconos'	=> $row3->menu_iconos,
												'childs'		=> array()
											);	
										$child_level_3[] = $menu3;	
									}					
	...
	public static function nestedMenu($parent=0,$position ='top',$active = '1')
	{
		...
		$active 	=  ($active =='all' ? "" : "AND activo ='1' ");
		$Q = DB::select("
		SELECT 
			tb_menu.*
		FROM tb_menu WHERE padre_id ='". $parent ."' ".$active." AND posicion ='{$position}'
		 ORDER BY orden			
		");					
		return $Q;					
	}
...
//	  $obj['grid'][1]['field']= 'padre_id';
//	  $obj['grid'][1]['label']= 'Padre Id';
//
//	  $obj['grid'][2]['field']= 'modulo';
//      $obj['grid'][2]['label']= 'Módulo';
//
//      $obj['grid'][4]['field']= 'menu_nombre';
//      $obj['grid'][4]['label']= 'Menú Nombre';
//
//      $obj['grid'][5]['field']= 'menu_tipo';
//      $obj['grid'][5]['label']= 'Menú Tipo';
//
//        $obj['grid'][6]['field']= 'rol_id';
//        $obj['grid'][6]['label']= 'Rol Id';
//
//        $obj['grid'][7]['field']= 'profundidad';
//        $obj['grid'][7]['label']= 'Profundidad';
//
//        $obj['grid'][8]['field']= 'orden';
//        $obj['grid'][8]['label']= 'Orden';
//
//        $obj['grid'][9]['field']= 'posicion';
//        $obj['grid'][9]['label']= 'Posicion';
//
//        $obj['grid'][10]['field']= 'menu_iconos';
//        $obj['grid'][10]['label']= 'Iconos';
//
//        $obj['grid'][11]['field']= 'activo';
//        $obj['grid'][11]['label']= 'Activo';
//
//        $obj['forms'][1]['field']= 'padre_id';
//        $obj['forms'][1]['label']= 'Padre Id';
//
//        $obj['forms'][2]['field']= 'modulo';
//        $obj['forms'][2]['label']= 'Módulo';
//
//        $obj['forms'][4]['field']= 'menu_nombre';
//        $obj['forms'][4]['label']= 'Menú Nombre';
//
//        $obj['forms'][5]['field']= 'menu_tipo';
//        $obj['forms'][5]['label']= 'Menú Tipo';
//
//        $obj['forms'][6]['field']= 'rol_id';
//        $obj['forms'][6]['label']= 'Rol Id';
//
//        $obj['forms'][7]['field']= 'profundidad';
//        $obj['forms'][7]['label']= 'Profundidad';
//
//        $obj['forms'][8]['field']= 'orden';
//        $obj['forms'][8]['label']= 'Orden';
//
//        $obj['forms'][9]['field']= 'posicion';
//        $obj['forms'][9]['label']= 'Posicion';
//
//        $obj['forms'][10]['field']= 'menu_iconos';
//        $obj['forms'][10]['label']= 'Iconos';
//
//        $obj['forms'][11]['field']= 'activo';
//        $obj['forms'][11]['label']= 'Activo';
//print        self::CF_encode_json($obj);
//        dd($obj);
      return $obj;
	}	
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
								...
								if($form['type'] =='hidden')
								{
									...
									$f .= '					
									  <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'>
										<label for="'.$form['label'].'" class=" control-label col-md-4 text-right"> '.$form['label'].' '.$required.'</label>
										<div class="col-md-6">
	...
	public static function formShow( $type , $field , $required ,$option = array()){
		...
		switch($type)
		{
			default;
				...
				if(isset($option['prefix']) && $option['prefix'] !='' or isset($option['sufix']) && $option['sufix'] !='')
				{
					...
					if($option['prefix'] !='')
						...
						{$mandatory}  {$attribute}   class='form-control input-sm {$extend_class}' />";
			...
			case 'textarea';
				if($required !='0') { $mandatory = 'required'; }
				$form = "<textarea name='{$field}' rows='5' id='{$field}' class='form-control input-sm {$extend_class}'  
				         {$mandatory} {$attribute} >{{ \$row['{$field}'] }}</textarea>";
				break;

			case 'textarea_editor';
				if($required !='0') { $mandatory = 'required'; }
				$form = "<textarea name='{$field}' rows='5' id='editor' class='form-control input-sm editor {$extend_class}'  
						{$mandatory}{$attribute} >{{ \$row['{$field}'] }}</textarea>";
				break;				
			...
			case 'text_date';
				...
				<div class=\"input-group m-b\" style=\"width:150px !important;\">
					{!! Form::text('{$field}', \$row['{$field}'],array('class'=>'form-control input-sm date')) !!}
			...
			case 'text_datetime';
				...
				$form = "
				<div class=\"input-group m-b\" style=\"width:150px !important;\">
					{!! Form::text('{$field}', \$row['{$field}'],array('class'=>'form-control input-sm datetime', 'style'=>'width:150px !important;')) !!}
					<span class=\"input-group-addon\"><i class=\"fa fa-calendar\"></i></span>
				</div>
				";
				break;				
			...
			case 'file';
				...
				if(isset($option['image_multiple']) && $option['image_multiple'] ==1)
				{
					...
					$form = "<input  type='file' name='{$field}' id='{$field}' class='inputfile  ";
					$form .= "@if(\$row['$field'] =='') class='required' @endif "; 
					$form .= "' {$attribute} />

							<label for='{$field}'><i class='fa fa-upload'></i> Choose a file</label>
							<div class='{$field}_preview'></div>
					 	<div >
	...
	public static  function bulkForm( $field, $forms = array(), $value ='')
	{
		...
		switch($type)
		{
			...
			case 'select';
				...
				$form = "<select name='$field{$bulk}'  class='form-control input-sm' $mandatory >
							<option value=''> -- Select  -- </option>
							$opts
						</select>";
				break;	
			...
			case 'radio';
				...
				$form = "<select name='$field{$bulk}' class='form-control input-sm' $mandatory ><option value=''> -- Select  -- </option>$opts</select>";
				break;												
	...
	public static function showUploadedFile($file,$path , $width = 80, $class = 'img-circle') {
		...
		//	echo $files ;
			...
			return "<img src='".asset('/uploads/images/no-image.png')."' border='0' width='".$width."' class=\"'.$class.'\" /></a>";
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			// FORMAT AS IMAGE
			$vals = '';
			$params = explode("|", $format_value);
            $class = isset($params[1])? $params[1] : 'img-circle';
			$values = explode(',',$value);
				...
				foreach($values as $val)
				{
					if($val != '')
					{
						if(file_exists('.'.$params[0] . $val))
						$vals .= '<a href="'.url( $params[0] . $val).'" target="_blank" class="previewImage"><img src="'.asset( $params[0] . $val ).'" border="0" width="30" class="'.$class.'" style="margin-right:2px;" /></a>';
					}
				}	 
		    $value = $vals;
	...
	{
		$path = base_path().'/resources/views/katana/modulo/template/';
		$fld = scandir($path);
	...
	public static function avatar( $width =75 , $id = '')
	{
		...
		$Q = DB::table("tb_usuarios")->where("id",'=',$id)->get();
		if(count($Q)>=1) 
		{
			$row = $Q[0];
			$files =  './uploads/usuarios/'.$row->avatar ;
			if($row->avatar !='' ) 	
			{
				if( file_exists($files))
				{
					return  '<img src="'.asset('uploads/usuarios').'/'.$row->avatar.'" border="0" width="'.$width.'" class="img-circle" />';
				} else {
					return $avatar;
				}	
			} else {
  ...
  static function ResumeUserStatus()
  {
  		$sql = 
  		"SELECT 
			SUM(CASE WHEN activo ='1' THEN '1' ELSE '0' END) AS s_active ,
			SUM(CASE WHEN activo='0' THEN '1' ELSE '0' END) AS s_inactive ,
			SUM(CASE WHEN activo ='2' THEN '1' ELSE '0' END) AS s_banned 
	...
	static public function buttonAction( $module , $access , $id , $setting)
	{
		...
		if($access['is_detail'] ==1) {
			if($setting['view-method'] != 'expand')
			{
				...
				$html .= '<li><a href="'.url($module.'/'.$id).'" '.$onclick.' class=" tips" title="'.__('core.btn_view').'"><i class="fa fa-search"></i> '.__('core.btn_view').' </a></li>';
			}
		}
		if($access['is_edit'] ==1) {
			$onclick = " onclick=\"ajaxViewDetail('#".$module."',this.href); return false; \"" ;
			...
			$html .= '<li><a href="'.url($module.'/'.$id).'/edit" '.$onclick.'  class="tips" title="'.__('core.btn_edit').'"><i class="fa  fa-edit"></i>  '.__('core.btn_edit').' </a></li>';
		}
		$html .= '</ul></div></div>';
		return $html;
	}	
	...
	static public function htmlExpandGrid()
	{
		...
		return View::make('katana.modulo.utility.extendgrid');
	}
	static public function userStatus( $stat)
	{
		if($stat =='1'){
			...
			return '<span class="label label-danger"> Baneado </span>';
		}
	}

    static public function entityStatus( $stat)
    {
        if($stat =='0'){
            return '<span class="label label-warning"> Inactivo </span>';
        }
