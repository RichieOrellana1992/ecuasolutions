...
class UserController extends Controller {
	...
	public function postCreate( Request $request) {
		...
		if ($validator->passes()) {
			...
			return Redirect::to('/login')->with('message',\SiteHelpers::alert('success',$message));
		} else {
	...
	public function getActivation( Request $request  )
	{
		...
		if($num =='')
			return Redirect::to('/login')->with('message',\SiteHelpers::alert('error','Invalid Code Activation!'));
		
		$user =  User::where('activation','=',$num)->get();
		if (count($user) >=1)
		{
			\DB::table('tb_users')->where('activation', $num )->update(array('active' => 1,'activation'=>''));
			return Redirect::to('/login')->with('message',\SiteHelpers::alert('success','Your account is active now!'));
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {	
			...
			if($user->count() >=1)
			{
				...
				return Redirect::to('/login?reset')->with('message', \SiteHelpers::alert('error','Cant find email address'));
			}

		}  else {
			return Redirect::to('/login?reset')->with('message', \SiteHelpers::alert('error','The following errors occurred')
			)->withErrors($validator)->withInput();
		}	 
	}	
	...
	public function getReset( $token = '')
	{
		...
		if($user->count() >=1)
		{
			...
			return Redirect::to('/login')->with('message', \SiteHelpers::alert('error','Cant find your reset code'));
		}
	...
	public function postDoreset( Request $request , $token = '')
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('/login')->with('message',\SiteHelpers::alert('success','Password has been saved!'));
		} else {
