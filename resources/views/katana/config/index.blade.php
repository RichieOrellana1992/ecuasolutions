...
@include('katana.config.tab')
	 {!! Form::open(array('url'=>'katana/config/save/', 'class'=>'form-horizontal row validated', 'files' => true)) !!}

	<div class="col-sm-6 animated fadeInRight ">
	  <div class="form-group">
	    <label for="ipt" class=" control-label col-md-4">{{ Lang::get('core.fr_appname') }} </label>
		<div class="col-md-8">
		<input name="cnf_appname" type="text" id="cnf_appname" class="form-control input-sm " required  value="{{ $ktnconfig['cnf_appname'] }}" />
		 </div> 
	  </div>  
	  ...
	  <div class="form-group">
		...
		<div class="col-md-8">
		<input name="cnf_appdesc" type="text" id="cnf_appdesc" class="form-control input-sm" value="{{ $ktnconfig['cnf_appdesc'] }}" />
		 </div> 
	  </div>  
	  ...
	  <div class="form-group">
		...
		<div class="col-md-8">
		<input name="cnf_comname" type="text" id="cnf_comname" class="form-control input-sm" value="{{ $ktnconfig['cnf_comname'] }}" />
		 </div> 
	  </div>      
	  ...
	  <div class="form-group">
		...
		<div class="col-md-8">
		<input name="cnf_email" type="text" id="cnf_email" class="form-control input-sm" value="{{ $ktnconfig['cnf_email'] }}" />
		 </div> 
	  </div>   
	  <div class="form-group">
		...
		<div class="col-md-8">
			<div class="">
				...
				@if($ktnconfig['cnf_multilang'] ==1) checked @endif
	   ...
	   <div class="form-group">
		...
		<div class="col-md-8">
				...
				@foreach(SiteHelpers::langOption() as $lang)
					...
					@if(config('ktn.cnf_lang') ==$lang['folder']) selected @endif
					>{{  $lang['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	   ...
	   <div class="form-group">
		...
		<div class="col-md-8">
				...
				@foreach(SiteHelpers::themeOption() as $t)
					...
					@if($ktnconfig['cnf_theme'] ==$t['folder']) selected @endif
					>{{  $t['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	   ...
	   <div class="form-group" style="display: none;">
		...
		<div class="col-md-8">
				...
				@foreach(SiteHelpers::backendOption() as $t)
					<option value="{{  $t['folder'] }}"
					@if($ktnconfig['cnf_backend'] ==$t['folder']) selected @endif
					>{{  $t['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	  ...
	  <div class="form-group hide">
		...
		<div class="col-md-8">
			<div class="checkbox">
				...
				@if ($ktnconfig['cnf_mode'] =='production') checked @endif
				  />  Production
			</div>
	...
	<div class="col-sm-6 animated fadeInRight ">
	  ...
	  <div class="form-group">
		...
		<div class="col-md-8">
			...
			foreach($dates as $key=>$val) {?>
				<option value="{{  $key }}"
				@if(config('ktn.cnf_date') ==$key) selected @endif
				>{{  $val }}</option>
	  ...
	  <div class="form-group">
		...
		<div class="col-md-8">
			<textarea class="form-control input-sm" name="cnf_metakey">{{ $ktnconfig['cnf_metakey'] }}</textarea>
		 </div> 
	  </div> 
	   ...
	   <div class="form-group">
		...
		<div class="col-md-8">
			<textarea class="form-control input-sm"  name="cnf_metadesc">{{ $ktnconfig['cnf_metadesc'] }}</textarea>
		 </div> 
	  </div>  
	   ...
	   <div class="form-group">
		...
		<div class="col-md-8">
			...
			<div style="padding:5px; border:solid 1px #243646 ; background:#243646 ; width:auto;">
			 	@if(file_exists(public_path().'/uploads/images/'.$ktnconfig['cnf_logo']) && $ktnconfig['cnf_logo'] !='')
			 	<img src="{{ asset('uploads/images/'.$ktnconfig['cnf_logo'])}}" alt="{{ $ktnconfig['cnf_appname'] }}" />
			 	@else
				<img src="{{ asset('uploads/logo.png')}}" alt="{{ $ktnconfig['cnf_appname'] }}" />
				@endif	
			</div>				
		 </div> 
	  </div>  		  
